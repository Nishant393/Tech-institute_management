...
const EditRecordedCourse = () => {
  ...
  const [sections, setSections] = useState([
    { 
      title: 'Section 1', 
      videos: [{ title: '', file: null, videoUrl: '', duration: '', uploading: false, progress: 0 }] 
    }
  ]);
    ...
    const fetchCourseData = async () => {
      try {
        ...
        course.videos.forEach(video => {
          const { title: videoTitle, videoUrl, duration, sectionTitle } = video;
          ...
          sectionMap.get(sectionTitle).videos.push({
            title: videoTitle,
            videoUrl,
            duration,
            file: null,
        ...
        // If no sections were created, keep the default empty section
  ...
  const uploadThumbnail = async (file) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://api.cloudinary.com/v1_1/draplsr9p/image/upload',
        formData,
        {
  ...
  const uploadVideo = async (sectionIndex, videoIndex, file) => {
    try {
      ...
      const response = await axios.post(
        'https://api.cloudinary.com/v1_1/draplsr9p/video/upload', 
        formData,
        {
          onUploadProgress: (progressEvent) => {
            const percentCompleted = Math.round((progressEvent.loaded * 100) / progressEvent.total);
      ...
      const updatedSections = [...sections];
      updatedSections[sectionIndex].videos[videoIndex].videoUrl = response.data.secure_url;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send the request
      await axios.put(`${server}recorded/update/${id.courseId}`, payload, {
        withCredentials: true,
      });
      ...
      // Redirect to course details page or course list
      navigate(`/recorded/content/${id.courseId}`); // Adjust path as needed
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-8">Edit Recorded Course</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                {thumbnailUploading ? 'Uploading...' : 'Change Thumbnail'}
              </button>
              {thumbnailPreview && (
                ...
                <div key={videoIndex} className="ml-4 mb-4 p-3 border-l-2 border-gray-200">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {video.videoUrl && !video.uploading && (
                    ...
                    {video.file && (
                      <div className="mt-2">
                        <p className="text-sm">
                          ...
                          {video.videoUrl && <span className="text-green-600 ml-2">Uploaded successfully!</span>}
                        </p>
                        ...
                        {video.uploading && (
                          <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                            ...
                            ></div>
                            <p className="text-xs text-gray-600 mt-1">{video.progress}% uploaded</p>
                          </div>
                        )}
                      </div>
                    )}
        ...
        <div className="flex justify-end space-x-4">
          <button
            type="button"
            onClick={() => navigate('/courses')} // Adjust path as needed
...
export default EditRecordedCourse;